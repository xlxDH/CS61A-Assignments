...
def boar_brawl(player_score, opponent_score):
    ...
    """
    t1 = player_score % 10
    t2 = opponent_score % 100 //10
    return max(3*abs(t1-t2),1)
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    # END PROBLEM 2
...
def sus_points(score):
    ...
    if num_factors(score)==3 or num_factors(score)==4:
        while is_prime(score)!=True: score = score+1
    return score
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    mn , idx , i = 0 , 1 , 1
    while i <= 10:
        aver = make_averaged(roll_dice,times_called)
        t = aver(i,dice)
        if t>mn:
            mn = t
            idx = i
